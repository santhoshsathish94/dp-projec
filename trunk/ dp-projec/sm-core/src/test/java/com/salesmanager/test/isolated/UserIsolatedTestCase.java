...
public class UserIsolatedTestCase {

    private static final Logger log = Logger.getLogger(UserIsolatedTestCase.class);
    ...
    /*
     * @Test public void addStaticContentData() throws ServiceException,
     * FileNotFoundException{
     * 
     * final MerchantStore store = merchantService.getByCode(
     * MerchantStore.DEFAULT_STORE ); final File file1 = new File(
     * "D:/wamp/www/travellingrants/wp-content/themes/TravelPro/style.css" );
     * 
     * if ( !file1.exists() || !file1.canRead() ) { throw new ServiceException(
     * "Can't read" + file1.getAbsolutePath() ); }
     * 
     * InputStaticContentData staticData=new InputStaticContentData();
     * staticData.setFileName( file1.getName() ); staticData.setFile( new
     * FileInputStream( file1 ) ); staticData.setContentType(
     * StaticContentType.STATIC_DATA );
     ...
     * }
     * 
     * @Test public void addStaticContentDataFiles() throws ServiceException,
     * FileNotFoundException{
     * 
     * List<InputStaticContentData> inputStaticContentDataList=new
     * ArrayList<InputStaticContentData>(); final MerchantStore store =
     * merchantService.getByCode( MerchantStore.DEFAULT_STORE ); final File
     * file1 = new File(
     * "D:/wamp/www/travellingrants/wp-content/themes/TravelPro/style.css" );
     * 
     * if ( !file1.exists() || !file1.canRead() ) { throw new ServiceException(
     ...
     * staticData.setFileName( file1.getName() ); staticData.setFile( new
     * FileInputStream( file1 ) ); staticData.setContentType(
     * StaticContentType.STATIC_DATA ); inputStaticContentDataList.add(
     * staticData );
     * 
     * final File file2= new File( "D:/personal/songs/Bahut der kardi.mp3" );
     * 
     * if ( !file2.exists() || !file2.canRead() ) { throw new ServiceException(
     * "Can't read" + file2.getAbsolutePath() ); }
     * 
     * InputStaticContentData staticData1=new InputStaticContentData();
     * staticData1.setFileName( file2.getName() ); staticData1.setFile( new
     * FileInputStream( file2 ) ); staticData1.setContentType(
     * StaticContentType.DIGITAL_DATA ); inputStaticContentDataList.add(
     * staticData1 ); staticContentService.addStaticContentDataFiles(
     * store.getCode(), inputStaticContentDataList );
     * 
     * }
     * 
     * @Test public void getStaticContentData() throws ServiceException{ final
     * MerchantStore store = merchantService.getByCode(
     * MerchantStore.DEFAULT_STORE ); String fileName="style.css";
     * OutputStaticContentData data=staticContentService.getStaticContentData(
     * store.getCode(), fileName ); System.out.println(data.getContentType());
     ...
     * 
     * List<Integer> groups = new ArrayList<Integer>(); User user =
     * userService.getByUserName("admin"); for(Group group : user.getGroups()) {
     * 
     * System.out.println(group.getGroupName()); groups.add(group.getId());
	...
	for (Group group : groups) {
	    if (group.getGroupName().equals("GROUP_SUPERADMIN") || group.getGroupName().equals("GROUP_ADMIN")) {
	...
	// final Product p = productService.getProductForLocale( 1L, language,
	// locale );
	 ...
	 * ids.add(3L); List<Product> products =
    ...
    @Test
    public void testCreateModules() throws ServiceException {
	...
	final MerchantStore store = testSupportFactory.createMerchantStore(MerchantStore.DEFAULT_STORE, country, currency, en);
