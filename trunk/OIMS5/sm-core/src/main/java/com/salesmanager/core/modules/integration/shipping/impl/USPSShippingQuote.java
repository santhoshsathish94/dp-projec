...
public class USPSShippingQuote implements ShippingQuoteModule {
	...
	public List<ShippingOption> getShippingQuotes(List<PackageDetails> packages, BigDecimal orderTotal, Delivery delivery, MerchantStore store, IntegrationConfiguration configuration,
		...
		System.out.println("====================In USPSShippingQuote========================= ");
		if (packages == null) {
			return null;
		}
		 ...
		 * !country.getIsoCode().equals("US")){ throw new
		 * IntegrationException("USPS Not configured for shipping in country " +
		 * country.getIsoCode()); }
		 */
		...
		/*
		 * if (!store.getCurrency().equals(Constants.CURRENCY_CODE_CAD)) { total
		 * = CurrencyUtil.convertToCurrency(total, store.getCurrency(),
		...
		try {
			 ...
			 * orderTotal, store.getCurrency(), Constants.CURRENCY_CODE_USD);
			 */
			...
			if (store.getCountry().getIsoCode().equals(delivery.getCountry().getIsoCode())) {
				...
				xmldatabuffer.append(size);
				xmldatabuffer.append("</Size>");
				xmldatabuffer.append("<Machinable>true</Machinable>");// TODO
																		// must
																		// be
																		// changed
																		// if
			...
			StringBuilder xmlbuffer = new StringBuilder().append(xmlheader.toString()).append(xmldatabuffer.toString()).append(xmlfooter.toString());
			...
			httpget = new GetMethod(protocol + "://" + host + ":" + port + completeUri);
			...
			if (result != 200) {
				LOGGER.error("Communication Error with usps quote " + result + " " + protocol + "://" + host + ":" + port + url);
			...
			// ID="1"><Error><Number>-2147218046</Number>
			...
			// Name</Description><HelpFile></HelpFile><HelpContext>1000440</HelpContext></Error></Package></IntlRateResponse>
			 ...
			 * ShippingConstants.NO_DISPLAY_RT_QUOTE_TIME; if (rtdetails !=
			 * null) {
			 * 
			 ...
			 * 
			 * returnOptions = new ArrayList<ShippingOption>(); // Map
			 * selectedintlservices = //
			 * (Map)config.getConfiguration("service-global-usps"); // need to
			 * create a Map of LABEL - LABLEL // Iterator servicesIterator = //
			 * selectedintlservices.keySet().iterator(); // Map services = new
			 * HashMap();
			 * 
			 * // ResourceBundle bundle = ResourceBundle.getBundle("usps", //
			 * locale);
			 * 
			 * // while(servicesIterator.hasNext()) { // String key =
			 * (String)servicesIterator.next(); // String value = //
			 ...
			 * .getShippingDescription()==ShippingDescription.LONG_DESCRIPTION)
			 * { if (option.getEstimatedNumberOfDays()>0) {
			 * description.append(" (").append(
			 * option.getEstimatedNumberOfDays()).append( " ").append( " d")
			 ...
			 * returnColl.add(option); // } returnOptions.add(option); }
			 * 
			 * // if(options.size()==0) { //
