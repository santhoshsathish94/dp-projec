...
public class UserController {
	...
	public @ResponseBody
	String pageUsers(HttpServletRequest request, HttpServletResponse response) {
	...
	private void populateUserObjects(User user, MerchantStore store, Model model, Locale locale) throws Exception {
		...
		/*
		 * if(user!=null && user.getId()!=null) { User logedInUser =
		 * userService.getByUserName(user.getAdminName());
		 * 
		 * //check groups List<Group> logedInUserGroups =
		 * logedInUser.getGroups(); for(Group group : logedInUserGroups) {
		 * if(group.getGroupName().equals(Constants.GROUP_SUPERADMIN)) { stores
		 * = merchantStoreService.list(); } } }
		 */
	...
	public @ResponseBody
	String checkUserCode(HttpServletRequest request, HttpServletResponse response, Locale locale) {
	...
	public String saveUser(@Valid @ModelAttribute("user") User user, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		ArrayList<Integer> submitedStoresId = new ArrayList<Integer>();
		if (user.getsMerchantStores().size() > 0) {
			for (String sMstore : user.getsMerchantStores()) {
				submitedStoresId.add(Integer.parseInt(sMstore));
			}
		}
		...
		List<MerchantStore> newStores = merchantStoreService.listStoreByIds(submitedStoresId);
		// set actual user groups
		user.setGroups(newGroups);
		user.setMerchantStores(newStores);
	...
	public @ResponseBody
	String removeUser(HttpServletRequest request, Locale locale) throws Exception {
	...
	public @ResponseBody
	String resetPassword(HttpServletRequest request, HttpServletResponse response, Locale locale) {

		AjaxResponse resp = new AjaxResponse();
		String userName = request.getParameter("username");

		/**
		 * Get User with userService.getByUserName Get 3 security questions from
		 * User.getQuestion1, user.getQuestion2, user.getQuestion3
		 */
	...
	public @ResponseBody
	String resetPasswordSecurityQtn(@ModelAttribute(value = "userReset") UserReset userReset, HttpServletRequest request, HttpServletResponse response, Locale locale) {
