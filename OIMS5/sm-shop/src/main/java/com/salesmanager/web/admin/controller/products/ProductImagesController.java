...
public class ProductImagesController {
	...
	public String displayProductImages(@RequestParam("id") long productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (attributes != null) {
			...
			for (ProductAttribute attribute : attributes) {
				if (!attribute.getProductOption().getProductOptionType().equalsIgnoreCase("text")) {
					productOptions.add(attribute.getProductOption());
				}
	...
	public @ResponseBody
	String pageProductImages(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// productAttributeService.getByProductId(store, product, language);

			List<ProductImage> images = new ArrayList<ProductImage>(product.getImages());
			...
			if (images != null) {
				...
				for (ProductImage image : images) {
					...
					String shade = "";
					if (!(image.getImage_product_option() == null)) {
						shade = productOptionService.getById(image.getImage_product_option()).getDescriptionsSettoList().get(0).getName();
					}				
					
					entry.put("shade", shade);
	...
	public String saveProductImages(@ModelAttribute(value = "productImages") @Valid final ProductImages productImages, final BindingResult bindingResult, final Model model,
			final HttpServletRequest request, Locale locale) throws Exception {
		...
		if (CollectionUtils.isNotEmpty(productImages.getFile())) {
			...
			for (final MultipartFile multipartFile : productImages.getFile()) {
				if (!multipartFile.isEmpty()) {
					...
					productImage.setDefaultImage(false);// default image is
														// uploaded in the
														// product details
					productImage.setImage_product_option(productImages.getImage_product_option());
		 ...
		 * productImages.getFile().get(0).getInputStream(); ImageContentFile
		 * cmsContentImage = new ImageContentFile();
		 * cmsContentImage.setFileName(
		 * productImages.getFile().get(0).getOriginalFilename() );
		 * cmsContentImage.setFile( inputStream );
		 * cmsContentImage.setFileContentType(FileContentType.PRODUCT);
		 * 
		 * productImage.setProductImage(productImages.getFile().get(0).
		 * getOriginalFilename() );
		 * 
		 * 
		 ...
		 * 
		 * productImageService.addProductImage(product, productImage,
		 * cmsContentImage);
		 */
	...
	public @ResponseBody
	String deleteImage(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		try {
			...
			// productAttributeService.getById(attributeId);

			/*
			 * if(attribute==null ||
			 * attribute.getProduct().getMerchantStore().getId
			 * ().intValue()!=store.getId()) {
			 * 
			 * resp.setStatusMessage(messages.getMessage("message.unauthorized",
			 * locale)); resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
			 * return resp.toJSONString(); }
			 * 
			 * 
